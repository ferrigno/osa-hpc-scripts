...
rm -rf obs/${OGID}J2
mkdir -p obs/${OGID}J2/pfiles
export PFILES="${PWD}/obs/${OGID}J2/pfiles;${ISDC_ENV}/pfiles:$HEADAS/syspfiles"
echo $PFILES
...
og_create \
   idxSwg=/tmp/$$.So.lis \
   ogid=${OGID}J2 baseDir=./ instrument=JMX2 obs_id=""
rm /tmp/$$.So.lis
...
#j_rebin_rmf ic_master=${REP_BASE_PROD}/idx/ic/${master_file} jemx_num=2 binlist=STD_008 outfile=jmx2_rebinned_rmf.fits
...
jemx_science_analysis ogDOL="${curr_dir}/og_jmx1.fits[1]" jemxNum=2 startLevel="COR" endLevel="LCR" \
...
IC_Alias="OSA" instMod="" response="jmx2_rebinned_rmf.fits" arf="" \
